...
`sprites.txt` is the source of every creature grid in the repo, and for 29 of the 30 species it is
also where the drawing was done by hand. Everything downstream is generated by
`generate_sprites.py`:
...
The exception is `slime`, whose 21 grids are struck by `drawer/` — see *The slime drawer* below.
The output still lands in `sprites.txt` as text, and nothing downstream knows the drawer exists.

`assets/` holds reference images. `blair-walk.jpg` is the Preston Blair walk cycle and is the
reference every stride in this repo is drawn against — see *Move follows Blair* below.
`slime-reference.png` is the 14x11 pixel-art slime the 72 grids take their proportions from.
Reference only: not build input, nothing reads them.
...
2-cell one as a 6px slab — which is why `slime` had no gloss until it moved to 72.
...
1. **Volume is now countable, so conserve it.** Every deformation of the creature — idle, move,
   sleep body, fight — carries 2174-2254 cells of silhouette against the base grid's 2224. A slime
   has a fixed volume, so what a pose loses in height it gains in width; draw a squash without
   conserving area and the thing inflates. Count the silhouette and not the ink: a closed eye removes
   fewer cells than an open one, so the sleep frames run 78 ink cells heavy on that alone. The
   attacks deliberately do not conserve it, and their comments say why.
...
3. **A curve costs nothing extra, so nothing is a staircase.** The dome, the sleeping lids and the
   wave's barrel are all curves at 1x. The same shapes at 24 were either straight or absent.
...
`slime`'s 21 grids are the one set struck by code rather than by hand. At 1x a pose is a curve and a
volume, and both are arithmetic: a superellipse profile, a squash that trades height for width, a
crescent that has to keep 3px of ink between itself and an eye that just moved. 21 frames of 72 rows
is more than anyone will re-derive by eye when a proportion changes, so the shapes live in `drawer/`
and `sprites.txt` keeps the result.

```bash
python3 tools/sprites/drawer/redraw_slime.py                 # check: does the file match the code?
python3 tools/sprites/drawer/redraw_slime.py --write          # rewrite the 21 grids in place
python3 tools/sprites/drawer/redraw_slime.py --preview p.png  # contact sheet (needs Pillow)
```

- `draw.py` — the primitives, the grid contract, and `validate()`, which checks what the parser does
  not: hole-to-hole and hole-to-outline clearance, and 1-2 cell specks.
- `base.py` — the knobs. `pose()` at its defaults *is* the base grid and also `idle.0`; there is
  deliberately no second code path for the neutral pose.
- `pose.py` / `frames.py` — the deformations and the four body states.
- `attacks.py` — rock, paper, scissors, which no transform produces.

Stdlib only, except `--preview`. Two things to know before touching it:

**Check the file, not the intermediate.** `generate_sprites.py --check` compares PNGs against grids
and passes happily when both are wrong together. `redraw_slime.py` with no arguments compares
`sprites.txt` against the code and names every block that drifted; that is the check that catches a
grid written from a half-finished edit. It is how the whole roster shipped once with no highlight at
all while every other check was green.

**The drawer does not write the notes.** The prose above each block in `sprites.txt` is hand-written
and nothing generates or verifies it. Change a shape here and the note is yours to correct — a note
claiming a 69-cell crescent over a grid that has none is worse than no note.
