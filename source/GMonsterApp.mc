...
class GMonsterApp extends Application.AppBase {
    ...
    function getInitialView() as [WatchUi.Views] or [WatchUi.Views, WatchUi.InputDelegates] {
        ...
        // Debug: launch straight into a battle instead of the pager, so the battle scene can be
        // looked at without walking to an encounter. The fight is a preview — it pays out nothing
        // and never touches the trek. Checked after the starter gate because a battle needs a
        // partner to field, and a fresh save has not chosen one yet.
        if (DebugConfig.INSTANT_BATTLE) {
            return Encounter.preview(DebugConfig.BATTLE_ENEMY, DebugConfig.BATTLE_IS_BOSS);
        }
